...
## 本地模拟器（不用真实飞书/多维表格也能试）

```bash
npm run sim
```

打开 `http://localhost:4173`，一个像飞书聊天界面一样的网页会启动。所有飞书 API、多维表格数据都是内存模拟的，不需要任何真实配置就能试指令效果：

- 输入 `@名字` 会被模拟成真实的飞书 @ 提及
- 多维表格数据存在内存里，创建的任务可以立刻用"我的任务"之类的指令查到
- 想测试 `排期` 功能的完整 LLM 流程，启动前设置环境变量：
  ```bash
  LLM_API_KEY=your-key npm run sim
  ```
  不设置的话，`排期` 会按预期提示"需要先配置 LLM_API_KEY"
- 右上角"重置状态"按钮会清空所有模拟数据，方便重新测试

这是纯本地开发工具（`sim/` 目录），不会被打包进部署的 Worker。
