...
export async function handleHelp(env: Env, chatId: string): Promise<void> {
  await replyText(env, chatId, [
    ...
    "批量创建任务：每行一个任务，同一行可写 类型 / 模块 / 版本 / ddl",
    "  每行末尾 @某人 设置该行的负责人（逐行独立，不会互相影响）",
    "  如果只在消息最后统一 @ 一个人、且某些行没单独 @，那些行会用这个人做默认负责人",
    "排期 <需求描述>：自然语言排一批功能，自动拆单、算依赖、排日期（需要配置 LLM_API_KEY）",
    "  例：排期 在3.1-3.30前排期一个月卡功能。人名可以直接打字，不用 @",
    "  机器人只在真正不确定时才会追问；回复「确认」创建草稿里的任务",
